...
async function archiveBoat(boatId: string): Promise<void> {
  ...
  const { rows } = await pool.query(
    `UPDATE boats
     ...
     WHERE id = $1
     RETURNING creator_user_id, archive_reason, unique_countries`,
    [boatId, nm],
  );
  console.log(`[journey] barco ${boatId} atracou em casa — ${nm} milhas náuticas`);

  // O fim da jornada era o único acontecimento importante do app que passava em
  // silêncio absoluto: o barco atracava, virava quadro no museu, e o dono só
  // descobria se fosse olhar. É o desfecho de semanas de viagem — merece ser
  // contado. Vai sem await: a varredura não pode emperrar por causa de um aviso.
  const b = rows[0];
  if (!b?.creator_user_id) return;
  try {
    const { rows: m } = await pool.query(
      `SELECT COUNT(*)::int AS n FROM boat_messages
        WHERE boat_id = $1 AND user_id <> $2`,
      [boatId, b.creator_user_id],
    );
    void avisar(b.creator_user_id, avisoVoltouParaCasa({
      motivo: b.archive_reason ?? 'perdido',
