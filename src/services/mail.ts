 ...
 * Envio de e-mail.
 *
 * O Adrift não tinha nenhum — a recuperação de senha é o primeiro caso. Em vez
 * de amarrar o código a um fornecedor, isto aqui é uma porta com duas saídas:
 *
 *   COM `RESEND_API_KEY`  manda de verdade, pela API do Resend (uma chamada
 *                         HTTPS, sem biblioteca nova no projeto).
 *   SEM a chave           escreve o e-mail no log do servidor, inteiro, com o
 *                         link clicável.
 *
 * A segunda saída não é preguiça: é o que permite testar o fluxo hoje, antes
 * de o domínio estar verificado no fornecedor. E deixa o dia de desenvolvimento
 * funcionando sem gastar cota nem mandar mensagem para ninguém de verdade.
 *
 * PARA LIGAR O ENVIO REAL:
 *   1. conta em resend.com e uma API key
 *   2. verificar o domínio adriftapp.fun (registros DNS na Hostinger)
 *   3. no Railway: RESEND_API_KEY=... e MAIL_FROM="Adrift <nao-responda@adriftapp.fun>"
 *
 * Sem o passo 2, o Resend só entrega para o e-mail do dono da conta, usando o
 * remetente onboarding@resend.dev — serve para testar, não para usuários.
 */
